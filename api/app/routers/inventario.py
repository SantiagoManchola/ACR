...
def listar_elementos(
    ...
    return db.execute(select(models.ElementoInventario)).scalars().all()
...
def entrada(
    ...
    svc_inventario.aplicar_movimiento(
        ...
        usuario.id, payload.motivo, payload.observaciones, fecha,
    )
    sellar(elem, usuario, nuevo=False)
    db.commit()
    mov = db.execute(
        select(models.MovimientoInventario).where(
            models.MovimientoInventario.elemento_id == elemento_id
...
@router.get("/alertas", response_model=list[ElementoOut], summary="Elementos bajo mínimo")
def alertas(
    db: Session = Depends(get_db),
    _: models.Usuario = Depends(require_role(_LECTORES)),
):
